...
class FlutterBleLib {
  ...
  Future<dynamic> cancelTransaction(String transactionId) async {
    await _mainMethodChannel.invokeMethod(_cancelTransaction, transactionId);
    return;
  }

  Future<dynamic> setLogLevel(LogLevel logLevel) => _mainMethodChannel
      .invokeMethod(_setLogLevel, LogLevelConverter.toMessage(logLevel).name);

  Future<LogLevel> logLevel() => _mainMethodChannel
      .invokeMethod(_logLevel)
      .then((logLevelByte) => bleData.LogLevelMessage.valueOf(logLevelByte))
      .then(
          (logLevelMessage) => LogLevelConverter.fromMessage(logLevelMessage));

  Future<BluetoothState> state() => _mainMethodChannel
      .invokeMethod(_state)
  ...
  Stream<BluetoothState> onStateChange() {
    Stream<BluetoothState> state = _bluetoothStateChanel
        .receiveBroadcastStream()
        .map((data) => bleData.BluetoothStateMessage.valueOf(data))
        .map((bluetoothStateMessage) {
      return BluetoothStateConverter.fromMessage(bluetoothStateMessage);
    });
  ...
  Future<BleDevice> connectToDevice(String macAddress,
      {bool isAutoConnect, int requestMtu}) async {
    ...
    return await _mainMethodChannel
        .invokeMethod(
            _connectToDevice, connectToDeviceDataMessage.writeToBuffer())
        .then((byteData) => new bleData.BleDeviceMessage.fromBuffer(byteData))
        .then((bleDeviceMessage) => BleDevice.fromMessage(bleDeviceMessage));
  }

  Future<bool> isDeviceConnected(String macAddress) async {
    final bool isConnected =
        await _mainMethodChannel.invokeMethod(_isDeviceConnected, macAddress);
    return isConnected;
  }

  Stream<BleDevice> onDeviceConnectionChanged() {
    return _deviceConnectionChange
        .receiveBroadcastStream()
        .map((data) => new bleData.BleDeviceMessage.fromBuffer(data))
        .map((bleDeviceMessage) => BleDevice.fromMessage(bleDeviceMessage));
  }

  Future<BleDevice> requestMTUForDevice(
      String deviceId, int mtu, String transactionId) async {
    return await _mainMethodChannel
        .invokeMethod(_requestMTUForDevice, <String, Object>{
          _deviceId: deviceId,
  ...
  Future<BleDevice> readRSSIForDevice(
      String deviceId, String transactionId) async {
    return await _mainMethodChannel
        .invokeMethod(_readRSSIForDevice, <String, String>{
          _deviceId: deviceId,
          _transactionId: transactionId,
        })
        .then((byteData) => new bleData.BleDeviceMessage.fromBuffer(byteData))
  ...
  Future<BleDevice> cancelDeviceConnection(String macAddress) async {
    return await _mainMethodChannel
        .invokeMethod(_cancelDeviceConnection, macAddress)
        .then((byteData) => new bleData.BleDeviceMessage.fromBuffer(byteData))
        .then((bleDeviceMessage) => BleDevice.fromMessage(bleDeviceMessage));
  }

  Future<BleDevice> discoverAllServicesAndCharacteristicsForDevice(
      String macAddress) async {
    return await _mainMethodChannel
        .invokeMethod(
            _discoverAllServicesAndCharacteristicsForDevice, macAddress)
        .then((byteData) => new bleData.BleDeviceMessage.fromBuffer(byteData))
        .then((bleDeviceMessage) => BleDevice.fromMessage(bleDeviceMessage));
  }
  ...
  Future<List<BleService>> servicesForDevice(String id) async {
    return await _mainMethodChannel
        .invokeMethod(_servicesForDevice, id)
        .then((byteData) => new bleData.ServiceMessages.fromBuffer(byteData))
        .then((serviceMessages) => new List.generate(
            serviceMessages.serviceMessages.length,
            (int index) => BleService.fromMessage(
                serviceMessages.serviceMessages[index])));
  }

  Future<List<Characteristic>> characteristicsForDevice(
          String deviceId, String serviceUUID) async =>
      await _invokeMethodCharacteristicFor(_characteristicsForDevice,
          <String, String>{_deviceId: deviceId, _serviceUUID: serviceUUID});

  Future<List<Characteristic>> characteristicsForService(
          double serviceIdentifier) async =>
      await _invokeMethodCharacteristicFor(_characteristicsForService,
          <String, Object>{_serviceIdentifier: serviceIdentifier});

  Future<List<Characteristic>> _invokeMethodCharacteristicFor(String methodName,
      [dynamic arguments]) {
    return _mainMethodChannel
        .invokeMethod(methodName, arguments)
        .then((byteData) =>
            new bleData.CharacteristicMessages.fromBuffer(byteData))
        .then((characteristicMessages) => new List.generate(
            characteristicMessages.characteristicMessage.length,
            (int index) => Characteristic.fromMessage(
                characteristicMessages.characteristicMessage[index])));
  }
  ...
  Future<Characteristic> _invokeMethodWriteCharacteristic(String methodName,
      [dynamic arguments]) {
    return _mainMethodChannel
        .invokeMethod(methodName, arguments)
        .then((byteData) =>
            new bleData.CharacteristicMessage.fromBuffer(byteData))
        .then((characteristicMessage) =>
            Characteristic.fromMessage(characteristicMessage));
  }
  ...
  Future<Characteristic> _invokeMethodReadCharacteristic(String methodName,
      [dynamic arguments]) {
    return _mainMethodChannel
        .invokeMethod(methodName, arguments)
        .then((byteData) =>
  ...
  Stream<MonitorCharacteristic> monitorCharacteristicForDevice(
          String deviceId,
          String serviceUUID,
          String characteristicUUID,
          String transactionId) =>
      _invokeMonitorCharacteristic(
          _monitorCharacteristicForDevice, <String, String>{
        _deviceId: deviceId,
        _serviceUUID: serviceUUID,
        _characteristicUUID: characteristicUUID,
        _transactionId: transactionId
  ...
  Stream<MonitorCharacteristic> monitorCharacteristicForService(
          ...
          String transactionId) =>
      _invokeMonitorCharacteristic(
          _monitorCharacteristicForService, <String, Object>{
        _serviceIdentifier: serviceIdentifier,
        _characteristicUUID: characteristicUUID,
        _transactionId: transactionId
      });

  Stream<MonitorCharacteristic> monitorCharacteristic(
          double characteristicIdentifier, String transactionId) =>
  ...
  Stream<MonitorCharacteristic> _invokeMonitorCharacteristic(String methodName,
